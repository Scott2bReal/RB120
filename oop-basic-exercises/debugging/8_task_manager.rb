...
# Because Valentina chose to name the local variable in which she was caching
# the selected array 'tasks' (the same name as the getter method for @tasks in
# TaskManager), because it had been declared but not assigned to a value yet,
# the value assigned to the local variable tasks at the time the select method
# is called on it is nil. This is an example of variable shadowing. The solution
# can be to directly reference the setter method as needed, or to rename
# the local variable something more specific, accurate, and different than the
# getter method
