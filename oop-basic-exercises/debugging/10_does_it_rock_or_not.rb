...
module DoesItRock
  ...
  class Score
    ...
    # rescue Exception
      ...
      NoScore.new # case statement uses is_a? for comparison, which needs an
                  # instance of the class vs. just the class itself.
    end
  end
...
# I would expect the AuthenticationError message to be displayed at some point.
# I made changes to the code to try to get it to work. It looks like the
# AuthenticationError is raised properly in SearchEngine#count, but it doesn't
# seem to persist throughout the program.
#
# I removed the rescue clause and that raised the error. Because the
# AuthenticationError was being rescued, it was always returning NoScore, which
# resulted in everything rocking.
